...
export default function CountriesAdmin() {
    ...
    try {
      ...
      if (editing) {
        ...
        if (created?.id || created?.Id) {
          setValue("id", created.id ?? created.Id);
        }
      }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const response = await deleteCountry(rowToDelete.id);
      ...
      if (response?.errors === null) {
        ...
        setError(response.errors?.data?.message || response.errors?.message);
      }
  ...
  return (
    <Box className="p-6 max-w-7xl mx-auto" sx={{ mt: "70px" }}>
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
          ...
          <Stack direction="row" spacing={1}>
            <IconButton onClick={loadData} disabled={loading}>
              {loading ? <CircularProgress size={20} /> : <LucideCloudy size={18} />}
            </IconButton>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          <Stack spacing={2} mt={1}>
            ...
            <Controller name="currencySymbol" control={control}
              render={({ field }) => <TextField {...field} label="Currency Symbol" fullWidth />}
            />

            <Controller name="isdCode" control={control}
              render={({ field }) => <TextField {...field} label="ISD Code" fullWidth />}
            />

            <Controller name="shortCode" control={control}
              render={({ field }) => <TextField {...field} label="Short Code" fullWidth />}
            />
          </Stack>
        </DialogContent>

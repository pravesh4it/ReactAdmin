...
import { GetPartners, AddPartner, UpdatePartner, DeletePartner} from "../../api/Partner";
...
const Partners = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                const response1 = await GetPartners();
    ...
    const handleSaveClient = async (data) => {
        try {
            if (selectedClient) {
                ...
                const response= await UpdatePartner(selectedClient.id, jsondata);
                if(response.errors==null)
                    {
                        setRowData((prev) =>
                ...
                const response = await AddPartner(jsondata);
                if(response.errors==null)
                {
    ...
    const columnDefs = [
        ...
        { headerName: 'Contact Person', field: 'contactPerson', flex: 2},
        { headerName: 'Contact No', field: 'contactNo1', flex: 2},
        { headerName: 'Email', field: 'email', flex: 2},
        //{ headerName: 'Variable', field: 'c_Variable', width: 150 },
        {
            headerName: 'Actions',
            field: 'actions',
            flex: 2,
    ...
    return (
        <>
            ...
            <Dialog open={openEditDialog} onClose={handleDialogClose}>
                <DialogTitle>{selectedClient ? "Edit Partner" : "Add Partner"}</DialogTitle>
                <DialogContent>
                    <form onSubmit={handleSubmit(handleSaveClient)}>
                        ...
                        {/* <Controller
                            ...
                            render={({ field }) => (
                                <TextField {...field} label="Variable" fullWidth margin="dense" error={!!errors.c_Variable} helperText={errors.c_Variable?.message} />
                            )}
                        />
                        */}
                            ...
                            <Controller
                                ...
                                rules={{  }}
                                render={({ field }) => (
                                    <TextField {...field} label="Success Link" fullWidth margin="dense" error={!!errors.successLink} helperText={errors.successLink?.message} />
                                )}
                            />
                            ...
                            <Controller
                                ...
                                rules={{  }}
                                render={({ field }) => (
                                    <TextField {...field} label="Quota Full Link" fullWidth margin="dense" error={!!errors.quotaFullLink} helperText={errors.quotaFullLink?.message} />
                                )}
                            />
                            <Controller
                                ...
                                rules={{  }}
                                render={({ field }) => (
                                    <TextField {...field} label="Disqualification Link" fullWidth margin="dense" error={!!errors.disqualificationLink} helperText={errors.disqualificationLink?.message} />
                                )}
                            />
                            <Controller
                                name="pausedLink"
                                ...
                                rules={{  }}
                                render={({ field }) => (
                                    <TextField {...field} label="Paused Link" fullWidth margin="dense" error={!!errors.pausedLink} helperText={errors.pausedLink?.message} />
                                )}
                            />
                            <Controller
                                ...
                                rules={{ }}
                                render={({ field }) => (
                                    <TextField {...field} label="Security Fail Link" fullWidth margin="dense" error={!!errors.securityFailLink} helperText={errors.securityFailLink?.message} />
                                )}
                            />
                        <DialogActions>
                            <Button onClick={handleDialogClose} color="primary">Cancel</Button>
